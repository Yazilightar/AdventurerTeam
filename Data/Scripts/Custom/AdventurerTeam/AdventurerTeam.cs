...
namespace Server.Scripts.Custom
{
    ...
    //    - Logic: Independent follower system (no BaseEscortable dependency).
    //    - Detection: Uses specific town list from TownGuards.cs.
    // ========================================================================
    public class CaptiveAdventurer : BaseCreature
    {
        ...
        public CaptiveAdventurer() : base(AIType.AI_Melee, FightMode.None, 10, 1, 0.2, 0.4)
        {
            ...
            // Initial State: Frozen and Invulnerable (Prevent death before rescue)
            CantWalk = true; 
            Blessed = true;  
            Frozen = true;   
        }

        // Check if any "Evil" Adventurers are nearby
        private bool CheckCaptorsNearby()
        {
            foreach (Mobile m in this.GetMobilesInRange(15))
            {
                if (m is AdventurerTeam)
                {
                    ...
                    // If any evil member is alive, the captive is too frightened to move
        ...
        {
            // If already rescued, double-click makes them follow closer
            if (m_IsRescued) 
            {
            ...
            if (CheckCaptorsNearby())
            {
                ...
                this.Animate(32, 5, 1, true, false, 0); // Bow/Tremble
        ...
        private void PerformRescue(Mobile rescuer)
        {
            ...
            // Set as follower (Bypassing pet slot limits)
            if (rescuer.Player)
            {
        ...
        private void CheckDestination()
        {
            ...
            // Safety check: Delete if master is missing
            if (ControlMaster == null || ControlMaster.Deleted || ControlMaster.Map != this.Map || !ControlMaster.InRange(this, 30))
            {
                this.Say("I am lost...");
                StopTimer();
            ...
            // Check if current location is a Safe Zone
        ...
        // ====================================================================
        // [CORE LOGIC] Safe Zone Detection
        // Derived directly from TownGuards.cs logic
        // ====================================================================
        private bool IsSafeZone(Region reg)
        {
            if (reg == null || reg.Map == Map.Internal) return false;

            // 1. Check Specific Town Names (Extracted from TownGuards.cs)
            string rName = reg.Name;
            if (!string.IsNullOrEmpty(rName))
            {
                // Exact matches from TownGuards.cs
                if (IndexOf(rName, "the Village of Whisper") || 
                    IndexOf(rName, "the Town of Glacial Hills") || 
                    IndexOf(rName, "the Village of Springvale") || 
                    IndexOf(rName, "the City of Elidor") || 
                    IndexOf(rName, "the Village of Islegem") || 
                    IndexOf(rName, "Greensky Village") || 
                    IndexOf(rName, "the Port of Dusk") || 
                    IndexOf(rName, "the Port of Starguide") || 
                    IndexOf(rName, "the Village of Portshine") || 
                    IndexOf(rName, "the Ranger Outpost") || 
                    IndexOf(rName, "the Lunar City of Dawn") || 
                    IndexOf(rName, "The Town of Devil Guard") || 
                    IndexOf(rName, "The Farmland of Devil Guard") || 
                    IndexOf(rName, "the Town of Moon") || 
                    IndexOf(rName, "the Village of Grey") || 
                    IndexOf(rName, "the City of Montor") || 
                    IndexOf(rName, "the Village of Fawn") || 
                    IndexOf(rName, "the Village of Yew") || 
                    IndexOf(rName, "Iceclad Fisherman's Village") || 
                    IndexOf(rName, "the Town of Mountain Crest") || 
                    IndexOf(rName, "Glacial Coast Village") || 
                    IndexOf(rName, "the Undercity of Umbra") || 
                    IndexOf(rName, "the City of Kuldara") || 
                    IndexOf(rName, "the Village of Barako") ||
                    // Classic UO Towns (Fallback)
                    IndexOf(rName, "Britain") || 
                    IndexOf(rName, "Minoc") || 
                    IndexOf(rName, "Trinsic") || 
                    IndexOf(rName, "Skara") || 
                    IndexOf(rName, "Magincia") || 
                    IndexOf(rName, "Jhelom") || 
                    IndexOf(rName, "Nujel") ||
                    IndexOf(rName, "Buccaneer"))
                {
                    return true;
                }

                // Generic keywords (Backup)
                if (IndexOf(rName, "Town") || IndexOf(rName, "City") || 
                    IndexOf(rName, "Guard") || IndexOf(rName, "Safe") || 
                    IndexOf(rName, "Village") || IndexOf(rName, "Market") ||
                    IndexOf(rName, "Bank") || IndexOf(rName, "Inn") || IndexOf(rName, "Tavern")) 
                {
                    return true;
                }
            }

            // 2. Check Region Type Name (Reflection)
            // Matches types found in World.cs NoApocalypse
            string typeName = reg.GetType().Name;
            if (typeName == "TownRegion" || 
                typeName == "GuardedRegion" || 
                typeName == "SafeRegion" || 
                typeName == "ProtectedRegion" || 
                typeName == "VillageRegion" || 
                typeName == "UmbraRegion" || 
                typeName == "LunaRegion" || 
                typeName == "BardTownRegion" || 
                typeName == "DawnRegion" || 
                typeName == "GargoyleRegion" || 
                typeName == "StartRegion" ||
                typeName == "PublicRegion")
            {
                return true;
            }
        ...
        {
            if (m_DestinationTimer != null)
            {
                m_DestinationTimer.Stop();
                m_DestinationTimer = null;
            }
        }
        ...
        {
            // Remove existing items manually
            for (int i = Items.Count - 1; i >= 0; --i)
            {
                Item item = Items[i];
                if (item is BaseClothing || item is BaseWeapon || item is BaseArmor)
                    item.Delete();
            }
    ...
    {
        #region 1. Text & Dialogue Data
        private static readonly string[] FriendlyGreetings = new string[] { "Safe travels.", "Hail." };
        private static readonly string[] EvilGreetings = new string[] { "Back off.", "This is our turf." };
        private static readonly string[] HeroicGreetings = new string[] { "Greetings, hero!", "An honor." };
        private static readonly string[] EvilJeerHero = new string[] { "Look at this 'hero'.", "You don't scare us." };
        private static readonly string[] FriendlyChat = new string[] { "Stay sharp.", "Did you hear that?" };
        private static readonly string[] EvilChat = new string[] { "Stop whining.", "Where is the gold?" };
        private static readonly string[] OutdoorChat = new string[] { "Nice weather.", "My feet hurt." };
        private static readonly string[] LootChat = new string[] { "Found something!", "Just trash." };
        private static readonly string[] CombatYell = new string[] { "Attack!", "Die!" };
        private static readonly string[] SquadDeathLines = new string[] { "Man down!", "You'll pay!" };
        private static readonly string[] PanicLines = new string[] { "I'm out of here!", "Run!" };
        private static readonly string[] VictoryLines = new string[] { "Too easy.", "Target eliminated." };
        private static readonly string[] RetreatLines = new string[] { "Retreat!", "Fall back!" };
        private static readonly string[] DeathLines = new string[] { "Argh...", "It ends here..." };
        private static readonly string[] HealSpellLines = new string[] { "In Vas Mani!", "Be healed!" };
        ...
        public override void OnThink()
        {
            ...
            if (now > m_NextEnvironmentCheck)
            {
                ...
                if (Combatant == null)
                {
                    ...
                    foreach (Item item in eable) {
                        if (item is Corpse && Utility.RandomDouble() < 0.2) {
            ...
            // 4. Self-Survival Logic
            if (now > m_NextSelfLogicTime)
            {
                if (Hits < HitsMax || Poisoned)
                {
                    DoSelfSurvival();
                    double cooldown = (m_CitizenType == (int)CitizenClass.Wizard) ? 4.0 : 8.0;
            ...
            // 6. Class Logic
            if (m_CitizenType == (int)CitizenClass.Rogue && !m_IsRetreating && Hits < HitsMax)
                Server.Misc.IntelligentAction.HideFromOthers(this);

            if (m_CitizenType == (int)CitizenClass.Fighter && Combatant != null && !m_IsRetreating)
            ...
            {
                if (GetAliveSquadCount() == 0 && Utility.RandomDouble() < 0.2)
                {
                    m_IsRetreating = true;
        ...
        private void DoSelfSurvival()
        {
            if (Poisoned)
            {
                if (m_CitizenType == (int)CitizenClass.Wizard)
                {
                    if (Mana >= 10)
                    {
                        ...
                        PlaySound(0x1E0); 
                        PublicOverheadMessage(MessageType.Emote, 0x3B2, true, "An Nox");
            ...
            if (Hits < HitsMax * 0.6)
            {
                int healAmount = Utility.RandomMinMax(20, 40);
                ...
                if (m_CitizenType == (int)CitizenClass.Wizard)
                {
                    if (Mana >= 15)
                    {
                        ...
                        Animate(17, 7, 1, true, false, 0);
                        PlaySound(0x1F2); 
                        PublicOverheadMessage(MessageType.Emote, 0x3B2, true, "In Vas Mani");
        ...
        private void PerformSquadSupport(DateTime now)
        {
            ...
            for (int i = m_MySquad.Count - 1; i >= 0; i--)
            {
                ...
                if (ally.Alive && ally.Map == this.Map && ally.InRange(this, 12) && CanSee(ally))
                {
                    ...
                    if (ally != this && ally.Hits < (ally.HitsMax * HealAllyThreshold))
                    {
                        target = ally;
                        isCure = false;
                    }
        ...
        private void CastSupportSpell(Mobile target, bool isCure)
        {
            ...
            {
                PlaySound(0x1E0);
                PublicOverheadMessage(MessageType.Emote, 0x3B2, true, GetPooledMessage(CureSpellLines));
            }
            else
            {
                PlaySound(0x1F2);
            ...
            Timer.DelayCall(TimeSpan.FromSeconds(1.5), delegate
            {
                if (!Deleted && Alive && target.Alive && target.Map == Map && target.InRange(this, 12))
                {
                    if (isCure)
                    {
                        target.CurePoison(this);
                        target.FixedParticles(0x373A, 10, 15, 5012, EffectLayer.Waist);
                    }
                    else
                    {
                        target.Heal(Utility.RandomMinMax(25, 45));
        ...
        private void ManageLightSource()
        {
            ...
            else if (twoHand is HeldLight) PackItem(twoHand);
        }

        // [IMPORTANT] Evil teams will not attack the captive.
        ...
        public override void OnMovement(Mobile m, Point3D oldLocation)
        {
            ...
            if (CanSendMessage(now) && CanSee(m) && m.InRange(this, 8))
            {
                ...
                {
                    Say(GetPooledMessage(m_IsEvil ? EvilGreetings : FriendlyGreetings));
                }
            }
        }
        ...
        public override void OnDamage(int amount, Mobile from, bool willKill)
        {
            ...
            {
                if ((double)Hits / HitsMax < RetreatThreshold && Utility.RandomDouble() < 0.35)
                {
                    m_IsRetreating = true;
                    m_RetreatResetTime = DateTime.UtcNow + TimeSpan.FromSeconds(6.0);
                    if (CanSendMessage(DateTime.UtcNow)) Say(GetPooledMessage(RetreatLines));
                    Combatant = null; Warmode = false;
                }
            }
        }
        ...
        {
            if (Utility.RandomDouble() < 0.75) Say(GetPooledMessage(DeathLines));
        ...
        private bool CanSendMessage(DateTime now) { if ((now.Ticks - m_LastMessageTime.Ticks) < SpeechThrottle.Ticks) return false; m_LastMessageTime = now; return true; }
        private string GetPooledMessage(string[] source) { if (source == null || source.Length == 0) return ""; return source[Utility.Random(source.Length)]; }
        ...
        public override void OnGaveMeleeAttack(Mobile defender) { base.OnGaveMeleeAttack(defender); if (m_CitizenType == (int)CitizenClass.Fighter && defender != null) Server.Misc.IntelligentAction.PunchStun(defender); }
        public override void OnCombatantChange() { base.OnCombatantChange(); if (Combatant != null && CanSendMessage(DateTime.UtcNow)) Say(GetPooledMessage(CombatYell)); else if (Combatant == null && !m_IsRetreating && CanSendMessage(DateTime.UtcNow)) Say(GetPooledMessage(VictoryLines)); }
        public override void OnAfterSpawn() { base.OnAfterSpawn(); if (Map != null) { this.Home = this.Location; this.RangeHome = DefaultHomeRange; Effects.SendLocationParticles(EffectItem.Create(this.Location, this.Map, EffectItem.DefaultDuration), 0x3728, 10, 10, 5023); PlaySound(0x1FE); } }
        public override void GenerateLoot() { if (m_CitizenLevel >= 7) AddLoot(LootPack.Rich); else if (m_CitizenLevel >= 5) AddLoot(LootPack.Average); else AddLoot(LootPack.Meager); if (Utility.Random(25) == 0) { Type t = Loot.AdventurerRareItemTypes[Utility.Random(Loot.AdventurerRareItemTypes.Length)]; Item r = Activator.CreateInstance(t) as Item; if (r != null) PackItem(r); } if (m_CitizenType == (int)CitizenClass.Wizard) AddLoot(LootPack.MedScrolls, (m_CitizenLevel / 3) + 1); }
        private void EnforceMountState(bool s) { if (s) { if (Mount == null) new Horse().Rider = this; } else { if (Mount != null) { IMount m = Mount; m.Rider = null; if (m is Mobile) ((Mobile)m).Delete(); } } }
        
        // [FULL VERSION] Restored full InitStatsAndAppearance
        private void InitStatsAndAppearance() { 
            Female = Utility.RandomBool(); Body = Female ? 401 : 400; Name = Female ? NameList.RandomName("female") : NameList.RandomName("male");
            if (!Female) FacialHairItemID = Utility.RandomList(0, 8254, 8255, 8256, 8257, 8267, 8268, 8269);

            m_CitizenLevel = Utility.RandomMinMax(1, 9); Fame = 2500 * m_CitizenLevel; Karma = m_IsEvil ? -Fame : Fame; VirtualArmor = m_CitizenLevel * 10;
            Title = m_IsEvil ? TavernPatrons.GetEvilTitle() : TavernPatrons.GetTitle(); Hue = m_IsEvil ? Utility.RandomList(0x995, 0x8A4, 0x8B0, 0x8AC) : Utility.RandomSkinHue(); FightMode = m_IsEvil ? FightMode.Good : FightMode.Evil;
            Utility.AssignRandomHair(this); SpeechHue = Utility.RandomTalkHue(); HairHue = FacialHairHue = Utility.RandomHairHue();
            ...
            if (type == 0) // Wizard
            { 
                IntelligentAction.DressUpWizards(this, false); m_CitizenType = 1; AI = AIType.AI_Mage; 
                SetSkill(SkillName.Psychology, baseSkill); SetSkill(SkillName.Magery, baseSkill); SetSkill(SkillName.Meditation, baseSkill);
                SetSkill(SkillName.MagicResist, baseSkill); SetSkill(SkillName.Tactics, baseSkill - 20);
                intMax += m_CitizenLevel * 30; 
            }
            else if (type == 1) // Fighter
            { 
                IntelligentAction.DressUpFighters(this, "", m_IsEvil, false, true); m_CitizenType = 2; AI = AIType.AI_Melee; 
                SetSkill(SkillName.Fencing, baseSkill); SetSkill(SkillName.Bludgeoning, baseSkill); SetSkill(SkillName.Swords, baseSkill);
                SetSkill(SkillName.Parry, baseSkill); SetSkill(SkillName.Tactics, baseSkill + 10); SetSkill(SkillName.Anatomy, baseSkill);
                strMax += m_CitizenLevel * 10; hitsMax += m_CitizenLevel * 20; 
            }
            else // Rogue
            { 
                IntelligentAction.DressUpRogues(this, "", m_IsEvil, false, true); m_CitizenType = 3; AI = AIType.AI_Archer; 
                SetSkill(SkillName.Marksmanship, baseSkill); SetSkill(SkillName.Tactics, baseSkill); SetSkill(SkillName.Hiding, baseSkill + 20);
                SetSkill(SkillName.Stealth, baseSkill + 20);
                dexMax += m_CitizenLevel * 10; 
            }
            
            foreach (Item item in Items) { if (item is BaseClothing && !(item is BaseArmor) && item.Hue == 0 && Utility.RandomBool()) item.Hue = Utility.RandomNeutralHue(); }
            
            SetStr(strMin, strMax); SetDex(dexMin, dexMax); SetInt(intMin, intMax); SetHits(hitsMin, hitsMax);
            
            AddWeapon(true);
            Server.Misc.IntelligentAction.GiveAdventureGear(this);
        }

        // [FULL VERSION] Restored weapon randomization
        public void AddWeapon(bool initial) { 
             ...
             else if (m_CitizenType == 1) // Wizard
             { 
             ...
             else if (m_CitizenType == 3) // Rogue
             { 
                 if (initial) { if(hand!=null) hand.Delete(); if(twohand!=null) twohand.Delete(); } 
                 
                 // 15% Chance for Throwing Gloves
    ...
    public static class AutoTeamMaintainer
    {
        ...
        {
            int nearbyCount = 0;
            foreach (Mobile m in pm.GetMobilesInRange(25)) { if (m is AdventurerTeam) nearbyCount++; }
            ...
            // 20% Chance for a Captive Mission (Evil Teams Only)
